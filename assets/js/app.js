...
class QuoteCalculator {
    ...
    initializeEventListeners() {
        ...
        const priceAffectingFields = [
            ...
            'antal_kallare_glugg', 'antal_1_luftare', 'antal_2_luftare', 'antal_3_luftare', 
    ...
    calculateExtrasCost(data) {
        ...
        if (data.hasSprojs && data.sprojsPerWindow > 0 && data.windowsWithSprojs > 0) {
            ...
            if (totalWindowCount > 0 && data.windowsWithSprojs > 0) {
                ...
                // Formel: pris per ruta × antal spröjs × genomsnittligt antal luftare × antal fönster med spröjs
                sprojsCost = pricePerRuta * data.sprojsPerWindow * avgLuftarePerWindow * data.windowsWithSprojs;
                console.log(`🖼️ Spröjs-beräkning: ${pricePerRuta}kr × ${data.sprojsPerWindow} spröjs × ${avgLuftarePerWindow.toFixed(1)} luftare/fönster × ${data.windowsWithSprojs} fönster med spröjs = ${sprojsCost}kr`);
            }
            
            console.log(`🖼️ Total spröjs-kostnad: ${sprojsCost}kr (${data.sprojsPerWindow <= CONFIG.EXTRAS.SPROJS_THRESHOLD ? '250kr' : '400kr'} per ruta)`);
            total += sprojsCost;
        }
