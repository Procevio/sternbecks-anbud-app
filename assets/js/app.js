...
const CONFIG = {
    ...
    WINDOW_TYPE_MULTIPLIERS: {
        ...
        'Isolerglas': 'per_window_400',            // +400kr per fönster (exkl. moms)
        'Kopplade isolerglas': 1.0,                // Separat fält i Google Forms
        'Insatsbågar yttre': 1.0,                  // Separat fält i Google Forms
        'Insatsbågar inre': 1.0,                   // Separat fält i Google Forms
        'Insatsbågar komplett': 1.0,               // Separat fält i Google Forms
        'Inåtgående': 0.95,                       // -5%
    ...
    // Tillägg (exkl. moms)
    EXTRAS: {
        SPROJS_LOW_PRICE: 150,      // 150kr per ruta för 1-3 spröjs (exkl. moms)
        SPROJS_HIGH_PRICE: 300,     // 300kr per ruta för över 3 spröjs (exkl. moms)
...
class QuoteCalculator {
    ...
    testSprojsCalculations() {
        ...
        const expected1 = 150 * 2 * 3 * 2; // 1,800kr
        console.log(`Test 1 - 2st 3-luftare med 2 spröjs: ${result1}kr (förväntat: ${expected1}kr) - ${result1 === expected1 ? 'PASS' : 'FAIL'}`);
        
        // Test 2: Högt pris (>3 spröjs)
        const testData2 = {
            luftare4: 4,
            hasSprojs: true,
            sprojsPerWindow: 4
        ...
        const expected2 = 300 * 4 * 4 * 4; // 19,200kr
        console.log(`Test 2 - 4st 4-luftare med 4 spröjs: ${result2}kr (förväntat: ${expected2}kr) - ${result2 === expected2 ? 'PASS' : 'FAIL'}`);
        
        // Test 3: Gränsvärde (=3 spröjs)
        const testData3 = {
            luftare2: 3,
            hasSprojs: true,
            sprojsPerWindow: 3
        ...
        const expected3 = 150 * 3 * 2 * 3; // 2,700kr
        console.log(`Test 3 - 3st 2-luftare med 3 spröjs: ${result3}kr (förväntat: ${expected3}kr) - ${result3 === expected3 ? 'PASS' : 'FAIL'}`);
        
        // Test 4: Blandade luftare-typer
        const testData4 = {
            luftare2: 2,
            luftare4: 1,
            hasSprojs: true,
        ...
        const expected4 = (300 * 5 * 2 * 2) + (300 * 5 * 4 * 1); // 6,000 + 6,000 = 12,000kr
        console.log(`Test 4 - Blandade luftare med 5 spröjs: ${result4}kr (förväntat: ${expected4}kr) - ${result4 === expected4 ? 'PASS' : 'FAIL'}`);
    ...
    updatePriceCalculation() {
        ...
        // Materialkostnad som ska dras av från totalen INKLUSIVE moms
        ...
        // ROT-avdrag beräknas på arbetskostnad (total efter materialavdrag)
    ...
    collectPricingData() {
        ...
        return {
            ...
            materialPercentage: getNumericValue('materialkostnad') || 10, // Standardvärde 10 om tomt
        ...
        data.windowTypes.forEach(windowType => {
            ...
            if (typeof multiplier === 'number') {
                if (windowType === 'Kopplade isolerglas') {
                    // 20% extra på totalen - beräknas på hela basepriset
                    totalCost += basePrice * (multiplier - 1);
                } else if (multiplier === 0) {
                    // Inget pris (insatsbågar yttre/inre/komplett)
                    totalCost += 0;
                } else {
                    // Procentuell ökning/minskning
                    totalCost += basePrice * (multiplier - 1);
                ...
                // Fast kostnad per fönster (isolerglas)
    ...
    calculateExtrasCost(data) {
        ...
        // Spröjs: Ny beräkningslogik baserad på antal luftare
        if (data.hasSprojs && data.sprojsPerWindow > 0) {
            let sprojsCost = 0;
            
            // Bestäm pris per ruta baserat på antal spröjs
            ...
            // Beräkna för varje typ av luftare
            const luftareTypes = [
                { count: data.luftare1, luftareCount: 1 },
                { count: data.luftare2, luftareCount: 2 },
                { count: data.luftare3, luftareCount: 3 },
                { count: data.luftare4, luftareCount: 4 },
                { count: data.luftare5, luftareCount: 5 },
                { count: data.luftare6, luftareCount: 6 }
            ];
            ...
            luftareTypes.forEach(({ count, luftareCount }) => {
                if (count > 0) {
                    ...
                    console.log(`🖼️ Spröjs ${luftareCount}-luftare: ${pricePerRuta}kr × ${data.sprojsPerWindow} spröjs × ${luftareCount} luftare × ${count} fönster = ${costForThisType}kr`);
                }
            });
            
            console.log(`🖼️ Total spröjs-kostnad: ${sprojsCost}kr (${data.sprojsPerWindow <= CONFIG.EXTRAS.SPROJS_THRESHOLD ? '150kr' : '300kr'} per ruta)`);
            total += sprojsCost;
        }
...
${data.hasRotDeduction ? `- ROT-avdrag (50% på arbetskostnad ${this.formatPrice(totalAfterMaterialDeduction)}): -${this.formatPrice(rotDeduction)}` : '- ROT-avdrag: Ej tillämpligt'}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
