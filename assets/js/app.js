...
const CONFIG = {
    ...
    FORM_FIELDS: {
        ...
        'är-du-berättigad-rot-avdrag': 'entry.1617253565' // Kund ROT-berättigad
    }
};
...
class QuoteCalculator {
    ...
    initializeEventListeners() {
        ...
        const priceAffectingControls = [
            'typ-av-renovering', 'typ-av-fonster', 'sprojs', 'byte-till-le-glas', 
            'fastighet-rot-berättigad', 'är-du-berättigad-rot-avdrag'
        ];
    ...
    updatePriceCalculation() {
        ...
        const subtotalBeforeMaterial = baseComponentsPrice + windowTypeCost + extrasCost + extraHoursCost;
        ...
        const subtotalBeforeRenovation = subtotalBeforeMaterial + materialCost;
    ...
    collectPricingData() {
        return {
            ...
            // ROT-avdrag
            propertyRotEligible: this.form.querySelector('input[name="fastighet-rot-berättigad"]:checked')?.value || '',
            customerRotEligible: this.form.querySelector('input[name="är-du-berättigad-rot-avdrag"]:checked')?.value || '',
            hasRotDeduction: this.form.querySelector('input[name="är-du-berättigad-rot-avdrag"]:checked')?.value === 'Ja - inkludera ROT-avdrag i anbudet'
        };
    }
    ...
    validateForm() {
        ...
        const radioGroups = [
            { name: 'typ-av-renovering', message: 'Vänligen välj typ av renovering' },
            { name: 'fastighet-rot-berättigad', message: 'Vänligen ange om fastigheten är berättigad ROT-avdrag' },
            { name: 'är-du-berättigad-rot-avdrag', message: 'Vänligen ange om kunden är berättigad ROT-avdrag' }
        ];
